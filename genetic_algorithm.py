...
class GeneticAlgorithm:
    def __init__(
        self,
        problem_instance: CVRPInstance,
        number_of_iterations: int,
        population_size: int,
        tournament_size: int,
        crossover_probbability: float = 0.7,
        mutation_probbability: float = 0.08,
        enable_immigrants: bool = False,
    ):
        ...
        self.original_tournament_size = self.tournament_size
        self.crossover_probbability = crossover_probbability
        self.mutation_probbability = mutation_probbability
        self.original_mutation_probability = self.mutation_probbability
    ...
    def _init_population(self) -> None:
        population = [
            CVRPSolution.generate_random(self.problem_instance)
    ...
    def _parent_selection(self) -> list[int]:
        ...
        for _ in range(self.population_size):
            ...
            fitness = [
                self.population[candidate].fitness for candidate in candidate_indices
            ]
    ...
    def _ox_crossover(self, parent_1: int, parent_2: int) -> list[int]:
        crossover_points = sorted(
            random.sample(range(1, self.problem_instance.dimension - 1), 2)
        )
        parent_1_section = self.population[parent_1].permutation[
            crossover_points[0] : crossover_points[1]
        ]

        parent_2_section_1_len = crossover_points[0]
        parent_2_section_2_len = (
            self.problem_instance.dimension - 1
        ) - crossover_points[1]
    ...
    def _mutation(self, child: list[int]) -> None:
        ...
        child[swap_points[0]], child[swap_points[1]] = (
            child[swap_points[1]],
            child[swap_points[0]],
        )
    ...
    def run(self):
        ...
        self.mutation_probbability = self.original_mutation_probability
        self.tournament_size = self.original_tournament_size
        self.stagnation_counter = 0
        self.fitness_history = []
        self.best_fitness = 0
        ...
        for idx in range(self.number_of_iterations):
            ...
            self.population = (
                elite + offspring_sorted[: self.population_size - self.elite_size]
            )
            ...
            if self.enable_immigrants:
                if idx > 0 and (
                    self.fitness_history[idx] < self.fitness_history[idx - 1]
                ):
                    self.stagnation_counter = 0
                else:
                ...
                if self.stagnation_counter >= 50:
                    ...
                    immigrants_size = int(self.population_size * 0.5)
                    immigrants = [
                        CVRPSolution.generate_random(self.problem_instance)
                        for _ in range(immigrants_size)
                    ]

                    self._evaluate(immigrants)

                    self.population[-immigrants_size:] = immigrants
                    self.mutation_probbability = self.mutation_probbability * 4
                    self.tournament_size = 2
                    self.stagnation_counter = 0
                    self.post_injection_counter = 0
                if hasattr(self, 'post_injection_counter'):
                    self.post_injection_counter += 1
                    if self.post_injection_counter == 30:
                        self.mutation_probbability = self.original_mutation_probability
                        self.tournament_size = self.original_tournament_size
                        del self.post_injection_counter
